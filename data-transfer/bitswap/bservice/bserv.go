...
func getBlock(ctx context.Context, c cid.Cid, bs blockstore.Blockstore, fget func() exchange.Fetcher) (blocks.Block, error) {
	...
	RunEnv.RecordMessage("error3: cid %v, fget : %v", c, fget)
	...
	if err == blockstore.ErrNotFound && fget != nil {
		...
		if err != nil {
			RunEnv.RecordMessage("error4: cid %v, err : %v", c, err.Error())
	...
	RunEnv.RecordMessage("error4-2: cid %v, err : %v", c, err.Error())
	if err == blockstore.ErrNotFound {
		return nil, ErrNotFound
	}
...
func getBlocks(ctx context.Context, ks []cid.Cid, bs blockstore.Blockstore, fget func() exchange.Fetcher) <-chan blocks.Block {
	...
	go func() {
		...
		for _, c := range ks {
			...
			if err != nil {
				RunEnv.RecordMessage("error5: cid %v, err : %v", c, err.Error())
		...
		if err != nil {
			RunEnv.RecordMessage("error6 %v", err.Error())
